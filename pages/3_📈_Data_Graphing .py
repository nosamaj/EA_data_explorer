...
def plot():
   ...
   df_data = eaapi.get_readings_sql(measure=measure,start_date=f"'{start_date}T00:00:00Z'", end_date=f"'{end_date}T23:59:59Z'")
   ...
   fig = px.line(st.session_state.measure_data, x="dateTime", y="value",title=f'Graph of selected measures grouped by type coloured by station', color="measure", facet_row="type", width=1280, height =680)
...
with st.form(key='form2', clear_on_submit=True):
  ...
  # for value in measure_values:
   measure =  df_measures['@id'][measure_value]
